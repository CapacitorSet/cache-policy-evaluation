...
keys = ("policy", "cache_type", "l1_size", "l2_size")
...
def process_single(values):
    ...
    output_filename = opts.prefix + entry.policy + "-" + entry.cache_type + "-" + entry.l1_size + "-" + entry.l2_size + ".txt"
...
with Pool(processes=opts.jobs) as pool:
    iterator = itertools.product(policies, cache_types, l1_sizes, l2_sizes)
